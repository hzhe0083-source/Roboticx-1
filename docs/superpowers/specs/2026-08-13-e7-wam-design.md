...
- 实现时需从基座 checkpoint 钉死的未知量（**不得硬编码**）：E7 实际
  action_dim（从 ckpt config 读，勿假设 7）、H11「最后时间片」的实际网格
  （dense 报 1152 tokens，需确认是 2 帧布局还是 24×24，取最后时间片后池化）、
  metric state 精确维度（p_flat × visibility 的 8D relation 格式）、cache
  实测规模（预算 14–20GB，超出则降池化档位）。
