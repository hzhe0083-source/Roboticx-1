...
- **动机（为什么预测世界）**：服务后续 harness 的自然语言服从性评测——VLA 只背「语言→动作」映射，对动作后果无概念；WAM 让「指令执行后的世界变化」可预测，动作残差朝该目标修正，语言服从从「背映射」升级为「因果对齐」。语言条件经 Qwen→VA→action condition 传入 WAM，无需新增输入。
