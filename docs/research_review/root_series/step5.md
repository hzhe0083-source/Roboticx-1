...
7. **Latency benchmark is incomplete.** `bench_inference.py` builds VA conditions before timing and measures only repeated `sample_actions`; it excludes VA-core encoding and V-JEPA. The >=10 Hz claim needs end-to-end p50/p95 measurement under the actual visual update path.
