...
def test_prepare_builds_three_episode_disjoint_p2_data_sides() -> None:
    ...
    assert "ACTION_HORIZON=${ACTION_HORIZON:-6}" in text
    assert "FLOW_TAIL_WEIGHT=1.0" in text
    assert "FLOW_TAIL_WEIGHT=0.036" in text
    assert "SOURCE=data/hard2_peer_h${ACTION_HORIZON}_p2_source_${DATA_TAG}.pt" in text
    assert "VA_TRAIN_DATA=data/hard2_peer_h${ACTION_HORIZON}_p2_va_train_${DATA_TAG}.pt" in text
    assert "WORLD_TRAIN_DATA=data/hard2_peer_h${ACTION_HORIZON}_p2_world_train_${DATA_TAG}.pt" in text
    assert "EVAL_DATA=data/hard2_peer_h${ACTION_HORIZON}_p2_eval_${DATA_TAG}.pt" in text
    assert "WORLD_POOL=data/hard2_peer_h${ACTION_HORIZON}_p2_world_pool_${DATA_TAG}.pt" in text
    assert "FAMILY=mw_hard2_va_world_state_exchange_joint_h${ACTION_HORIZON}_p2_${DATA_TAG}" in text
    assert '--horizon "$ACTION_HORIZON"' in prepare
    assert "--planning-stride 2" in prepare
    assert '--data-contract "$DATA_CONTRACT"' in prepare
    ...
    assert '"$ACTION_HORIZON" "$WORLD_HORIZON" "$DATA_CONTRACT"' in preflight
    assert 'episodes[left] & episodes[right]' in preflight
    assert '(("va", "world"), ("va", "eval"), ("world", "eval"))' in preflight
    assert 'get("current_action_prefix_steps") != world_horizon' in preflight
...
def test_peer_cli_requires_joint_dual_data_streams() -> None:
    ...
    assert '"--wmrm-cycle-steps", sys.argv[3]' in preflight
...
def test_joint_run_uses_both_data_streams_without_phase_handoff() -> None:
    ...
    assert '--wmrm-cycle-steps "$WORLD_HORIZON"' in joint
    assert "--flow-prefix-steps 2" in joint
    assert '--flow-tail-weight "$FLOW_TAIL_WEIGHT"' in joint
    ...
    assert '--resume-weights-migration "$RESUME_WEIGHTS_MIGRATION"' in joint
...
def test_checkpoint_contract_records_joint_gradient_and_data_protocol() -> None:
    ...
    assert '"peer_world_topology": "world_minus_one_same_endpoint_fixed_current_anchor_v2"' in block
    assert '"peer_training_mode": "joint_dual_stream"' in block
    assert '"peer_gradient_boundary": "world_map_stopgrad_policy_projection_trainable_v1"' in block
    ...
    assert '"wmrm_cycle_steps": (config.get("wmrm_cycle_steps"), world_horizon)' in block
    assert '"contract": data_contract' in block
