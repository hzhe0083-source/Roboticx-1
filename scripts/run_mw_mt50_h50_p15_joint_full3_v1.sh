...
LONGTRAJ_DECODE_CACHE_TASKS=${LONGTRAJ_DECODE_CACHE_TASKS:-50}
PEER_PREFETCH_DEPTH=${PEER_PREFETCH_DEPTH:-1}
EXPECTED_EPISODES=${EXPECTED_EPISODES:-2500}
ONLINE_RECOVERY_SAMPLES=${ONLINE_RECOVERY_SAMPLES:-0}
TRAIN_DINO=${TRAIN_DINO:-1}
WMRM_FEATURE_METRIC=${WMRM_FEATURE_METRIC:-mse}
MODEL_LR=${MODEL_LR:-0.00001}
WMRM_PREDICTOR_LR=${WMRM_PREDICTOR_LR:-}
RUN_ID=${RUN_ID:-mw_mt50_h50_p15_joint_va_wm_dinoall_separatepcgrad_from_s313_e3_b32_v1}
...
read_steps(){
  "$PY" -B - "$ONLINE_INDEX" "$BATCH" "$SAMPLES_PER_EPISODE" "$EPOCHS" "$EXPECTED_EPISODES" <<'PY'
import json
import sys
from pathlib import Path

index = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
batch, samples_per_episode, epochs, expected_episodes = map(int, sys.argv[2:])
episodes = len(index.get("episodes") or [])
if episodes != expected_episodes:
    raise SystemExit(f"expected {expected_episodes} episodes, got {episodes}")
...
preflight(){
  ...
  [[ "$BATCH" == 32 ]] || fail "joint full-DINO run requires global batch 32"
  [[ "$NGPUS" == 2 ]] || fail "joint full-DINO run requires two GPUs"
  [[ "$TRAIN_DINO" == 0 || "$TRAIN_DINO" == 1 ]] || fail "TRAIN_DINO must be 0 or 1"
  (( LONGTRAJ_DECODE_CACHE_TASKS > 0 )) || fail "LONGTRAJ_DECODE_CACHE_TASKS must be positive"
  (( PEER_PREFETCH_DEPTH > 0 )) || fail "PEER_PREFETCH_DEPTH must be positive"
  (( ONLINE_RECOVERY_SAMPLES >= 0 && ONLINE_RECOVERY_SAMPLES <= SAMPLES_PER_EPISODE )) || \
    fail "ONLINE_RECOVERY_SAMPLES must be in [0,$SAMPLES_PER_EPISODE]"
...
PY
  ...
  printf 'joint preflight: PASS; baseline=43.6%% episodes=%s ' "$EPISODES"
  printf 'samples/epoch=%s recovery/episode=%s steps/epoch=%s epochs=%s total_steps=%s batch=%s dino_train=%s metric=%s lr=%s cache=%s prefetch=%s encode_batch=%s\n' \
    "$SAMPLES" "$ONLINE_RECOVERY_SAMPLES" "$STEPS_PER_EPOCH" "$EPOCHS" "$STEPS" "$BATCH" \
    "$TRAIN_DINO" "$WMRM_FEATURE_METRIC" "$MODEL_LR" "$LONGTRAJ_DECODE_CACHE_TASKS" \
    "$PEER_PREFETCH_DEPTH" "$MAIN_VISION_ENCODE_BATCH"
}
...
PY
    ...
    if [[ -n "${RUN_STEPS_OVERRIDE:-}" ]]; then
      (( RUN_STEPS_OVERRIDE > 0 && completed > 0 )) || \
        fail "extension requires positive RUN_STEPS_OVERRIDE and checkpoint step"
    else
  ...
  if [[ "$launch_mode" == exact ]]; then
    printf 'exact resume: phase_step=%s run_steps=%s target_step=%s\n' \
      "$completed" "$run_steps" "$((completed + run_steps))"
  fi
  local save_args=(--save-every "$STEPS_PER_EPOCH" --save "$SAVE")
  local vision_train_args=()
  local predictor_lr_args=()
  if [[ "$TRAIN_DINO" == 1 ]]; then
    vision_train_args=(--vision-unfreeze-all --lr-vision 0.000001)
  fi
  if [[ -n "$WMRM_PREDICTOR_LR" ]]; then
    predictor_lr_args=(--lr-wmrm-predictor "$WMRM_PREDICTOR_LR")
  fi
  if [[ "${NO_SAVE:-0}" == 1 ]]; then
    save_args=()
  fi
  ...
  PYTHONUNBUFFERED=1 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 \
    ...
    "$PY" -m torch.distributed.run --standalone --nproc_per_node="$NGPUS" \
      ...
      --online-recovery-samples-per-episode "$ONLINE_RECOVERY_SAMPLES" \
      ...
      --task-locality-block-batches 1 \
      --longtraj-decode-cache-tasks "$LONGTRAJ_DECODE_CACHE_TASKS" \
      ...
      --main-vision-checkpoint "$DINO" "${vision_train_args[@]}" \
      ...
      --peer-batch-prefetch-depth "$PEER_PREFETCH_DEPTH" \
      ...
      --flow-tail-weight 1.0 --lr "$MODEL_LR" "${predictor_lr_args[@]}" \
      --steps "$run_steps" \
      "${save_args[@]}" \
