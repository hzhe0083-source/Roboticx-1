...
退化目标（测试集 Δg 逐元素方差均值 < 1e-12 或 err_base < 1e-12）判 INVALID
（exit 3）；improvement ≤ 0 时 shuffle 门不判（判据 0.5×improvement ≤ 0 恒真），
整体直接 FAIL。

输出：三误差与两门判定 PASS/FAIL。
exit code：0 = PASS（两门全过），2 = FAIL（任一门不过，或 improvement≤0 时
shuffle 门不判），3 = 数据不足或退化目标。
...
用法：
  ...
  python scripts/probe_action_dependency.py --synthetic --degenerate   # 期望 exit 3
    ...
    for split in ("train", "val", "test"):
        ...
        for i in range(len(ds)):
            ...
            if valid is not None and torch.as_tensor(valid).numel() == 1 and not bool(torch.as_tensor(valid).item()):
                continue
            # E7 审查修复：perturbed_future[0]=True 的样本其 +6 几何目标被人为
            # 扰动（cache 注入的反事实），不能作为 G1 干净标签；M0 合成模式
            # 无此字段，不受影响。
            pf = rec.get("perturbed_future")
            if pf is not None:
                try:
                    first = pf[0] if len(pf) > 0 else pf
                except TypeError:
                    first = pf
                flag = bool(first.item()) if hasattr(first, "item") else bool(first)
                if flag:
                    continue
...
def main() -> int:
    ...
    ap.add_argument("--degenerate", action="store_true", help="合成模式：动作与 Δg 全零的退化自检（期望 INVALID，exit 3）")
    ...
    if args.degenerate and not args.synthetic:
        ap.error("--degenerate 需要 --synthetic")

    if args.synthetic:
        x, dg, ep_ids, task_ids = _build_synthetic(args.n, args.zero_correlation, args.seed)
        if args.degenerate:
            # 退化自检入口：动作与 Δg 全零 → 测试集目标方差 0 → G1 INVALID exit 3。
            x = torch.zeros_like(x)
            dg = torch.zeros_like(dg)
        src = f"synthetic (zero_correlation={args.zero_correlation}, seed={args.seed}, n={args.n}, degenerate={args.degenerate})"
    else:
        try:
    ...
    # E7 审查修复：退化目标先验拒绝。测试集 Δg 全零（或 err_base≈0）时
    # err_base≈err_action≈err_shuffle≈0，两门同真（Codex 实证的全零双 PASS
    # 边界）——判 INVALID（exit 3）而非 PASS。
    dg_var_test = float(y_test.var(dim=0).mean())
    if dg_var_test < 1e-12 or err_base < 1e-12:
        print(
            f"G1 INVALID: degenerate targets "
            f"(test Δg per-element mean variance={dg_var_test:.3e}, "
    ...
    if improvement <= 0.0:
        # 无改进时 0.5*improvement ≤ 0，「shuffle_gain ≥ 0.5*improvement」在
        # 微小正差下恒真——shuffle 门不判，整体直接 FAIL。
        shuffle_gain = None
        gate_shuffle = None
    else:
    ...
    if shuffle_gain is None:
        print("  shuffle_gain               = n/a  "
              "(improvement <= 0, gate not judged)  -> n/a")
    else:
        print(f"  shuffle_gain               = {shuffle_gain:.6f}  "
              f"(need >= {0.5 * improvement:.6f})  -> {'PASS' if gate_shuffle else 'FAIL'}")

    if bool(gate_improvement) and bool(gate_shuffle):
