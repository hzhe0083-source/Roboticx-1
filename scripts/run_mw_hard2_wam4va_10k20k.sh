#!/usr/bin/env bash
# Two-task long-horizon WAM4VA: single-stage joint.
#   0  assembly-v3
#   16 door-unlock-v3
# JEPA world + VA flow. Handshake on. No --wmrm-only.
# Writes every 1k step copy.
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo "===== joint ${JOINT_STEPS} steps batch=${JOINT_BATCH} (JEPA world + VA flow) =====" | tee "$LOG"
